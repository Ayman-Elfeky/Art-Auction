...
{
    Task<string> CreateBid(Api.Models.Bid input);
    Task<List<Api.Models.Bid>> GetBids(string Id);
}
...
public class BidService : IBidService
{
    ...
    public BidService(ApplicationDbContext context, ICurrentUserContext currentUserContext, IHubContext<AuctionHub> hubContext)
    {
        _context = context;
    ...
    public async Task<string> CreateBid(Api.Models.Bid input)
    {
        ...
        var artwork = await _context.Artworks.Include(a => a.Artist).FirstOrDefaultAsync(a => a.Id == artworkId)
        ...
        var highestBid = await _context.Bids.Where(b => b.ArtworkId == artworkId).OrderByDescending(b => b.Amount).FirstOrDefaultAsync();
        ...
        {
            Id = Guid.NewGuid(), ArtworkId = artworkId, BuyerId = buyerId,
            Amount = input.Amount, PlacedAt = DateTime.UtcNow, IsWinning = true
        };

        if (highestBid != null) { highestBid.IsWinning = false; _context.Bids.Update(highestBid); }
        ...
        await _hubContext.Clients.Group(AuctionHub.GetArtworkGroup(artworkId.ToString()))
            .SendAsync("bid.placed", new { artworkId, bidderId = buyerId, bidderName = buyer.Username, amount = bidEntity.Amount, timestamp = bidEntity.PlacedAt });

        if (artwork.Status == ArtworkStatus.Ended)
        {
            await _hubContext.Clients.Group(AuctionHub.GetArtworkGroup(artworkId.ToString()))
                .SendAsync("auction.ended", new { artworkId, winnerId = buyerId, winnerName = buyer.Username, finalPrice = bidEntity.Amount });
        }

        return bidEntity.Id.ToString();
    }

    public async Task<List<Api.Models.Bid>> GetBids(string Id)
    {
        ...
        var bids = await _context.Bids
            ...
            .ToListAsync();
        return bids.Select(b => new Api.Models.Bid(
            b.Id.ToString(),
            b.BuyerId.ToString(),
