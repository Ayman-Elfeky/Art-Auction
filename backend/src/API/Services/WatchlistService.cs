...
public sealed class WatchlistService : IWatchlistService
{
    ...
    public async Task<string> AddToWatchlist(string artworkId)
    {
        ...
        if (!artworkExists) throw new InvalidOperationException("Artwork not found.");

        var exists = await _context.Watchlists.AnyAsync(w => w.BuyerId == buyerId && w.ArtworkId == parsedArtworkId);
        if (exists) return "Artwork is already in watchlist.";

        _context.Watchlists.Add(new ArtAuction.Domain.Entities.Watchlist
        {
            Id = Guid.NewGuid(), BuyerId = buyerId, ArtworkId = parsedArtworkId, AddedAt = DateTime.UtcNow
        });
    ...
    public async Task<SuccessMessage> RemoveFromWatchlist(string artworkId)
    {
        ...
        var item = await _context.Watchlists.FirstOrDefaultAsync(w => w.BuyerId == buyerId && w.ArtworkId == parsedArtworkId);
        if (item is null) return new SuccessMessage(true, "Artwork is not in watchlist.");
    ...
    public async Task<List<Artwork>> GetMyWatchlist()
    {
        ...
        return await _context.Watchlists
            ...
            .Select(w => new Artwork(
                w.Artwork.Id, w.Artwork.Title, w.Artwork.Artist.Username,
                w.Artwork.Category.Name, (double)w.Artwork.InitialPrice,
                (double)w.Artwork.CurrentBid, w.Artwork.AuctionEndTime,
                w.Artwork.Status.ToString(), w.Artwork.ImageUrl))
            .ToListAsync();
    }
}
